...
<!-- End Title & Breadcrumbs -->
                                        ...
                                        @if ($rekap->nomor_user == $user->nomor && $rekap->tanggal == $date->format('Y-m-d'))
                                                <span class="badge badge-circle {{$rekap->class_badge}} badge-sm">
                                                    <a href="#" class="text-gray-200" 
                                                        onclick="infoRekap('{{ $rekap->id }}')">{{$rekap->tipe_schedule}}</i>
                                                    </a>
                                                </span>
                                        @endif
                                    @endforeach
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    ...
                    <form id="kt_modal_add_rekap_form" class="form" method="POST">
                        ...
                        <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_add_rekap_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_rekap_header" data-kt-scroll-wrappers="#kt_modal_add_rekap_scroll" data-kt-scroll-offset="300px">
                            ...
                            <div class="input-group input-group-sm" style="margin-right: 2px;">
                                <input class="form-control form-control" placeholder="From Date" id="kt_daterangepicker_3" name="from_date_rekap" tabindex="5"/>
                            </div>
                            <!--end::Input-->
                            </div>
                            <!--begin::Input-->                            
                            ...
                            <div class="input-group input-group-sm" style="margin-right: 2px;">
                                <input class="form-control form-control" placeholder="From Date" id="kt_daterangepicker_4" name="to_date_rekap" tabindex="6"/>
                            </div>
                            <!--end::Input-->
                            </div>
                            <!--end::Input group-->
                        ...
                        <div class="text-center pt-15">    
                                ...
                                <button type="button" class="btn btn-custom-purple" data-kt-rekap-modal-action="submit" id="buttonInsertRekap">
    ...
    <div class="modal fade" id="kt_modal_info_rekap" tabindex="-1" aria-hidden="true">
        <!--begin::Modal dialog-->
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <!--begin::Modal content-->
            <div class="modal-content">
                <!--begin::Modal header-->
                <div class="modal-header" id="kt_modal_info_rekap_header">
                    <!--begin::Modal title-->
                    <h2 class="fw-bold" id="modalTittle">Info Rekap Presensi</h2>
                    <!--end::Modal title-->
                    <!--begin::Close-->
                    <div class="btn btn-icon btn-sm btn-active-icon-primary"  data-bs-dismiss="modal" aria-label="Close">
                        <!--begin::Svg Icon | path: icons/duotune/arrows/arr061.svg-->
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    <!--begin::Form-->
                    <form id="kt_modal_info_rekap_form" class="form" action="#" method="POST">
                        @csrf
                        <!--begin::Scroll-->
                        <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_info_schedule_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_info_schedule_header" data-kt-scroll-wrappers="#kt_modal_info_schedule_scroll" data-kt-scroll-offset="300px">
                            <!--begin::Input group-->
                            <input type="text" id="rekap_id" name="rekap_id" value="{{ old('rekap_id') }}" hidden/>
                            ...
                            <div class="input-group mb-5">
                                ...
                                <input type="text" class="form-control" id="schedule_clock_in" name="schedule_clock_in" value="{{ old('schedule_clock_in') }}" aria-describedby="basic-addon3" readonly/>
                            </div>
                            <div class="input-group mb-5">
                                <span class="input-group-text" id="basic-addon3">Schedule Clock Out</span>
                                <input type="text" class="form-control" id="schedule_clock_out" name="schedule_clock_out" value="{{ old('schedule_clock_out') }}" aria-describedby="basic-addon3" readonly/>
                            </div>
                            <div class="input-group mb-5">
                                <span class="input-group-text" id="basic-addon3">Presensi Clock Out</span>
                                <input type="text" class="form-control" id="presensi_clock_in" name="presensi_clock_in" value="{{ old('presensi_clock_in') }}" aria-describedby="basic-addon3" readonly/>
                            </div>
                            <div class="input-group mb-5">
                                <span class="input-group-text" id="basic-addon3">Presensi Clock Out</span>
                                <input type="text" class="form-control" id="presensi_clock_out" name="presensi_clock_out" value="{{ old('presensi_clock_out') }}" aria-describedby="basic-addon3" readonly/>
                            </div>
                            <div class="input-group mb-5">
                                <span class="input-group-text" id="basic-addon3">Keterangan</span>
                                <input type="text" class="form-control" id="keterangan" name="keterangan" value="{{ old('keterangan') }}" aria-describedby="basic-addon3" readonly/>
                            </div>
                            <div class="input-group mb-5">
                                <span class="input-group-text" id="basic-addon3">Total Jam Kerja</span>
                                <input type="text" class="form-control" id="total_jam_kerja" name="total_jam_kerja" value="{{ old('total_jam_kerja') }}" aria-describedby="basic-addon3" readonly/>
                            </div>
                            <!--end::Input group-->
...
<script>
    $(document).ready(function () {
        $('#kt_modal_info_rekap').modal({backdrop: 'static', keyboard: false})
        $('#kt_modal_add_rekap').modal({backdrop: 'static', keyboard: false})
    });
    ...
    function infoRekap(id){   
        $.ajax({
        url: "/getonerekap/"+id,
        type: 'GET',
        success: function(data) {
            $.each(data, function(index, item) {
                $('#rekap_id').val(item.id);
                $('#nama').val(item.nama);
                $('#nomor_user').val(item.nomor_user);
                $('#tanggal').val(item.tanggal);
                $('#schedule_clock_in').val(item.schedule_clock_in);
                $('#schedule_clock_out').val(item.schedule_clock_out);
                $('#presensi_clock_in').val(item.clock_in);
                $('#presensi_clock_out').val(item.clock_out);
                $('#keterangan').val(item.keterangan_status);
                $('#total_jam_kerja').val(item.total_jam_kerja);
            });
        }
        });

        $('#kt_modal_info_rekap').modal('show');
    }
