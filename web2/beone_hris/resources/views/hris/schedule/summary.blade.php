    ...
    <div id="kt_content_container" class="d-flex flex-column-fluid align-items-start container-xxl">
        ...
        <div class="content flex-row-fluid" id="kt_content">
            ...
            <div class="card">
                ...
                <form method="GET" action="/summarySchedule">
                            ...
                            <tr>
                                ...
                                @foreach ($date_range as $date)
                                    ...
                                    @foreach($listschedule as $schedule)
                                        @if ($schedule->nomor_user == $user->nomor && $schedule->tanggal == $date->format('Y-m-d'))
                                                <!-- <a href="#" class="fas fa-check-circle text-success" onclick="infoSchedule('{{ $schedule->id }}')"></a> -->
                                                <span class="badge badge-circle {{$schedule->tipe_schedule}} badge-sm">
                                                    <a href="#" class="text-gray-200" 
                                                        onclick="infoSchedule('{{ $schedule->id }}')">{{$schedule->tipe_schedule}}</i>
                                                    </a>
                                                </span>
                                                <!-- <a href="#" class="fas fa-times-circle text-danger" onclick="infoSchedule('{{ $schedule->id }}')"></a> -->
                                        @endif
                                    @endforeach
                                    </td>
                                @endforeach
    ...
    function infoSchedule(id){   
        ...
        success: function(data) {
            $.each(data, function(index, item) {
                ...
                $('#clock_in').val(item.clock_in);
                $('#clock_out').val(item.clock_out);
            });
        }
        });

        $('#kt_modal_info_schedule').modal('show');
    }
...
<script src="{{asset('assets/js/custom/apps/hris/validation_summary_schedule.js')}}"></script>
@endsection