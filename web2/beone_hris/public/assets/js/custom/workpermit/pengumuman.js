...
function editpengumuman(idpengumuman) {
    ...
    $.ajax({
        type: "GET",
        url: uri + "/getonepengumuman" ,
        data: { idpengumuman: idpengumuman },
        dataType: 'json',
        success: function(res) {
            console.log(res);
            $('#idpengumumanedit').val(res['iduser']);
            $('#titleedit').val(res['title']);
            $('#descedit').val(res['desc']);
            $("#idsectionedit").val(res['dept']).change();
            selectElement('idsectionedit',res['dept']);
            $('#edtavatar').css('background-image', "url('"+ uri + '/' + res['avatar'] +"')");
            document.getElementById('fileA').setAttribute("href", uri + '/' + res['attachment']);
...
function insert(){
    const t = document.getElementById("kt_modal_add_user");
    const i = t.querySelector('[data-kt-users-modal-action="submit"]');
    const url = uri + "/insertpengumuman";
    fetch(url, {
        method : "POST",
